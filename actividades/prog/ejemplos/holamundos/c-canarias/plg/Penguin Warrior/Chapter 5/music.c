...
/* We'll set this to 1 as soon as we start playback, and to 0 when there's no
   more data. */
int music_playing = 0;
...
void UpdateMusic()
{
	...
	/* If we have a buffer of data, append it to the playback buffer.
	   ...
	   format for the data. This prevents OpenAL from converting stereo 
	   data
	...
	if (buf_count != 0) {
	    ...
	    /* Check for (unlikely) errors. If something went wrong, disable 
	       music. */
