...
typedef struct net_pkt_s {
    ...
    Uint8 fire;          /* Player sending this packet is firing.
			    For simplicity, we'll only allow the
			    player to fire once per packet. */ 
...
typedef struct net_link_s {
    ...
    struct sockaddr_in addr; /* Peer's address structuure. */
...
/* Fills in a network game packet based on a player's current information.
   firing indicates that the local player is firing and that the remote
     end should display a projectile.
