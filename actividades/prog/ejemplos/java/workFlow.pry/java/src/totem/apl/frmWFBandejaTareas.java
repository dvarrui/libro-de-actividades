/*-----------------------------------------------
   Fichero  :  frmWFBandejaTareas.java
   Fecha    :  10-06-2003
   Estado   :  Desarrollo (Menu+Panel de botones)
-----------------------------------------------*/
...
/**
 * Aplicación de bandeja de tareas
 * 
 * @author David Vargas Ruiz
 * @version 0.8.0
 */
class frmWFBandejaTareas extends JFrame 
{
   //--------------------
   //Componentes Visuales
   //--------------------
   JButton	  btnPaginaSiguiente, btnVerDetalle, btnSalir;
   ...
   //=======================
   frmWFBandejaTareas(frmWFLogin p_frmlogin, WFSesion p_sesion) 
   {
      ...
      logger = new WFRegistrarSesion(p_sesion); logger.ponModulo("frmWFBandejaTareas");
   }
   ...
   class ClaseBotonListener implements ActionListener 
   {
	 ...
	 else if (strAccion == "CrearNuevoTiquet")  { procBtnCrearNuevoTiquet();}
   ...
   private void crearMenu()
   {
      ...
      menu = new JMenu("Tiquet");     menuBarra.add(menu);
      menuItem = new JMenuItem("Burcar registros");  menuItem.setActionCommand("Buscar"); 
      menuItem.addActionListener(btnListener);       menu.add(menuItem);
      menuItem = new JMenuItem("Ver detalle");       menuItem.setActionCommand("VerDetalle"); 
      menuItem.addActionListener(btnListener);       menu.add(menuItem);
      menuItem = new JMenuItem("Crear nuevo");       menuItem.setActionCommand("CrearNuevoTiquet"); 
      menuItem.addActionListener(btnListener);       menu.add(menuItem);
      
      menu = new JMenu("Ver");     menuBarra.add(menu);
      menuItem = new JMenuItem("Página anterior");   menuItem.setActionCommand("PaginaAnterior"); 
      menuItem.addActionListener(btnListener);       menu.add(menuItem);
      menuItem = new JMenuItem("Página siguiente");  menuItem.setActionCommand("PaginaSiguiente"); 
      menuItem.addActionListener(btnListener);       menu.add(menuItem);
      menu.addSeparator();
      menuItem = new JMenuItem("Ocultar criterios"); menuItem.setActionCommand("OcultarCriterios"); 
      menuItem.addActionListener(btnListener);       menu.add(menuItem);
      menuItem = new JMenuItem("Mostrar criterios"); menuItem.setActionCommand("MostrarCriterios"); 
      menuItem.addActionListener(btnListener);       menu.add(menuItem);
      menu.addSeparator();
   ...
   private void crearContentPane()
   {
      ...
      if (sesion.leeValorDeOpcion("APL.BANDEJA.VER-CRITERIOS").equals("S")) panelCriterios.setVisible(true);
      if (sesion.leeValorDeOpcion("APL.BANDEJA.VER-BOTONERA").equals("S")) panelHorizontal.setVisible(true);
   ...
   private void crearPanelCriterios()
   {
	 ...
	 lbNumPagina.setForeground(Color.red);
      ...
      panelCriterios.add(new JLabel("Tiquet ",JLabel.RIGHT));     panelCriterios.add(edCodTicket);
   ...
   private void crearPanelHorizontal()
   {
	 ...
	 JButton btn;
         btn = new JButton("Buscar"); btn.setMnemonic('b');
         btn.setActionCommand("Buscar"); btn.setToolTipText("Buscar registros seleccionados");
	 btn.addActionListener(btnListener);panelHorizontal.add(btn);
	 
         btn = new JButton("Anterior"); btn.setMnemonic('a');
         btn.setActionCommand("PaginaAnterior"); btn.setToolTipText("Ir a la página anterior");
	 btn.addActionListener(btnListener); panelHorizontal.add(btn);
	 ...
	 btnPaginaSiguiente.setToolTipText("Ir a la página siguiente");
         ...
         btnVerDetalle.setActionCommand("VerDetalle"); btnVerDetalle.setToolTipText("Ver datos del tiquet seleccionado");
   ...
   private void crearPanelTabla()
   {
      ...
      for (int i = 0; i < 4; i++) 
      {
         ...
         if (i<2) {  columna.setPreferredWidth(8); }
   ...
   private boolean procBtnBuscar()
   {
      try
      { 
         if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnBuscar()");
         filtro.desactivar();
   ...
   //==========================
   // procBtnCrearNuevoTiquet()
   //==========================
   private boolean procBtnCrearNuevoTiquet()
   {
      if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnCrearNuevoTiquet()");	     
      return false;	   
   }
   ...
   {
      if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnPaginaAnterior()");	     
   ...
   {
      if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnPaginaSiguiente()");	     
      int i = filtro.leeNumPaginaActual();
   ...
   {
      if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnOcultarCriterios()");	     
      panelCriterios.setVisible(false);//this.pack(); 
      this.setSize(prop.getInt("Tamano_Largo"),prop.getInt("Tamano_Alto")); 
      sesion.ponValorDeOpcion("APL.BANDEJA.VER-CRITERIOS","N");
      return true;
   }
   ...
   {
      if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnMostrarCriterios()");	     
      panelCriterios.setVisible(true);//this.pack();    
      this.setSize(prop.getInt("Tamano_Largo"),prop.getInt("Tamano_Alto")); 
      sesion.ponValorDeOpcion("APL.BANDEJA.VER-CRITERIOS","S");
      return true;	     
   }
   ...
   { 
      if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnOcultarBotonera()");	     
      panelHorizontal.setVisible(false); this.setSize(prop.getInt("Tamano_Largo"),prop.getInt("Tamano_Alto")); 
      sesion.ponValorDeOpcion("APL.BANDEJA.VER-BOTONERA","N");return true;
   }
   ...
   { 
      if (sesion.leeValorDeParametro("BD.MONITORIZAR").equals("S")) logger.registrarLOG("procBtnMostrarBotonera()");	           panelHorizontal.setVisible(true); this.setSize(prop.getInt("Tamano_Largo"),prop.getInt("Tamano_Alto")); 
      sesion.ponValorDeOpcion("APL.BANDEJA.VER-BOTONERA","S");return true;
   }
   ...
   private boolean procBtnVerDetalle()
   {
	 ...
	 WFTicket ticket=filtro.leeTicketConCodigo(i);
		 ...
		 logger.registrarLOG("procBtnVerDetalle(tiquet="+ticket.leeCodTicket()+")");
	 if (appTicket==null)  {appTicket = new frmWFTicket(this,ticket);}
	 else                  { appTicket.ponTiquet(ticket);}
   ...
   public WFFiltro leeFiltro()  {return filtro;}
   ...
   class MiModeloDeTabla extends AbstractTableModel 
   {
      ...
      final String[] columnNames = {"CODIGO","PADRE","F.CREACION","TIPO","TAREA"};
      private Object data[][];
	 ...
	 System.out.println("New value of data:"); printDebugData();
      ...
      private void printDebugData() 
      {
         ...
         { 
            System.out.print(" Fila[" + i + "]"); for(int j=0;j<numCols;j++) {System.out.print("  "+data[i][j]);} 
	    System.out.println();
         }
         System.out.println("--------------------------");
      }
