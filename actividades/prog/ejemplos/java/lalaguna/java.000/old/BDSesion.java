 ...
 * de la base de datos (objeto del tipo <b>BDConexion</b>)
 *
 * @author David Vargas Ruiz
 * @version 0.1.0
 */
public class BDSesion extends Object
{
   ...
   public void inicializar(BDConexion p_bd)
   {
      // Crear sesion con la BBDD de WF
      usuario=null; clave=null; cod_perfil=-1; fec_desde=null; fec_hasta=null;
      fecha_login=null;intentos_login=0;
   ...
   public boolean login(String p_usuario, String p_clave)
   {
      ...
      Connection con = bd.leeConnection();
      Statement st1,st2;
      ResultSet rs1,rs2;
      ...
      if(++intentos_login>3)
      { cod_estado=ERROR; des_estado= new String("ERROR Sesion.login()"); return false; }
	 ...
	 if (rs1.next())
            ...
            if (fecha_login.after(fec_desde) && (fec_hasta==null||fecha_login.before(fec_hasta)))
	    {
	       ...
	       rs2 =st2.executeQuery("SELECT MAX(FECHA) as LAST_LOGIN FROM LOG_D_ACCESOS WHERE TIPO='LOG' "
		     +"AND MODULO='SESION.LOGIN' AND USUARIO='"+usuario+"'");
	       ...
	       st2.executeUpdate("INSERT INTO LOG_D_ACCESOS (TIPO,USUARIO,PC,MODULO,DESCRIPCION) VALUES('LOG','"
		     +usuario+"','"+pc+"','SESION.LOGIN','Realizado login')");
	       st2.close();
	    ...
	    else
	    {
	       ...
	       st2.executeUpdate("INSERT INTO LOG_D_ACCESOS (TIPO,USUARIO,PC,MODULO,DESCRIPCION) VALUES('WRN','"
		     +usuario+"','"+pc+"','SESION.LOGIN','Intento "+intentos_login+" fallido (Usuario desactivado)')");
	       st2.close();
	    }
	 }
	 else
	 {
	    ...
	    st2 = con.createStatement();
	    st2.executeUpdate("INSERT INTO LOG_D_ACCESOS (TIPO,USUARIO,PC,MODULO,DESCRIPCION) VALUES('WRN','"
	       +p_usuario+"','"+pc+"','SESION.LOGIN','Intento "+intentos_login+" fallido (Usuario/clave incorrecto)')");
	    st2.close();
	 }
	 rs1.close();
	 st1.close();
      ...
      { 
	cod_estado=ERROR; des_estado=new String("ERROR Sesion.login()");
        System.err.println("Exception en Sesion.login(): " + e);
      }
   ...
   public boolean logout()
   {
      try
      {
	    ...
	    st1.executeUpdate("INSERT INTO LOG_D_ACCESOS (TIPO,USUARIO,PC,MODULO,DESCRIPCION) VALUES('LOG','"
	       +usuario+"','"+pc+"','SESION.LOGOUT','Realizado logout')");
	    st1.close();
	    //Limpiar las variables
	    usuario=null; clave=null; cod_perfil=-1;bd=null;fec_desde=null;fec_hasta=null;pc=null;
	    fecha_login=null;fecha_last_login=null;nombre=null;apellido1=null;apellido2=null;
      ...
      { 
	 cod_estado=ERROR; des_estado = new String("ERROR Sesion.logout()");
	 System.err.println("ERROR Sesion.logout(): " + e);
      }
      return false;
   }
   ...
   public Connection     leeConnection()     { if (cod_estado==LOGIN) return bd.leeConnection(); return null; }
   ...
   {  
      if (cod_estado==LOGIN)
      {
         Connection con = bd.leeConnection();  Statement st1;  ResultSet rs1;
         try
         {
	    st1 = con.createStatement();
	    ...
	    if (rs1.next()) {rs1.close();st1.close(); return true;}
         ...
         { 
	    cod_estado=ERROR; des_estado=new String("ERROR Sesion.tienePermiso()");
            System.err.println("Exception en Sesion.tienePermiso(): " + e);
         }
      }
      return false;
   }
   ...
   {  
      if (cod_estado==LOGIN)
      {
	    ...
	    if (rs1.next()) {String s=new String(rs1.getString("VALOR")); rs1.close();st1.close();return s;}
         ...
         { 
	    cod_estado=ERROR; des_estado=new String("ERROR Sesion.leeValorDeParametro()");
            System.err.println("Exception en Sesion.leeValorDeParametro(): " + e);
         }
      }
      return "*";
   }
   ...
   {  
      if (cod_estado==LOGIN)
      {
	    ...
	    if (rs1.next()) {String s=new String(rs1.getString("VALOR")); rs1.close();st1.close();return s;}
         ...
         { 
	    cod_estado=ERROR; des_estado=new String("ERROR Sesion.leeValorDeOpcion()");
            System.err.println("Exception en Sesion.leeValorDeOpcion(): " + e);
         }
      }
      return "*";
   }
   ...
   {  
      if (cod_estado==LOGIN)
      {
         Connection con = bd.leeConnection(); Statement st1;
         try
         {
	    ...
	    return true;
         ...
         { 
	    cod_estado=ERROR; des_estado=new String("ERROR Sesion.ponValorDeOpcion("+p_opcion+","+p_valor+")");
            System.err.println("Exception en Sesion.ponValorDeOpcion(): " + e);
         }
      }
      return false;
   }
   ...
   //===================================================
   public BDRegistroPredicado crearBDRegistroPredicado()
   {
      if (cod_estado==LOGIN)
      {   BDRegistroPredicado r = new BDRegistroPredicado(); r.inicializar(this); return r; }
      return null;
   }
}
//Fin de la clase BDSesion.java
